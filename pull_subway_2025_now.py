...
DATASET_ID = "5wq4-mkjj"  # MTA Subway Hourly Ridership: Beginning 2025
BASE_URL = f"https://data.ny.gov/resource/{DATASET_ID}.json"

START_MONTH = "2025-01-01"
END_MONTH = None

OUTPUT_DIR = Path("data/raw/mta_subway")
...
def pull_one_month(month_start: pd.Timestamp) -> pd.DataFrame:
    ...
    params = {
        ...
        "$where": f"""
            transit_mode = 'subway'
            AND transit_timestamp >= '{start_str}'
        ...
        "$group": """
            ...
            payment_method
        """,
        "$order": "transit_timestamp, station_complex_id, payment_method",
        "$limit": LIMIT,
    }
...
def main():
    ...
    for month_start in months:
        ...
        output_path = OUTPUT_DIR / f"subway_hourly_station_{ym}.parquet"
