...
INPUT = "data/processed/bus_did_panel.parquet"

OUT_RESULTS = "outputs/models/10b_event_study_bus_results.txt"
OUT_FIGURE = "outputs/figures/10b_event_study_bus.png"
OUT_COEFS = "outputs/models/10b_event_study_bus_coefficients.csv"
OUT_COV = "outputs/models/10b_event_study_bus_covariance.csv"

os.makedirs("outputs/models", exist_ok=True)
os.makedirs("outputs/figures", exist_ok=True)

print("=" * 90)
print("Loading Bus DiD panel...")
print("=" * 90)
...
df = df[df["bus_ridership"] >= 0].copy()
df["log_bus_ridership"] = np.log1p(df["bus_ridership"])
...
print("=" * 90)
print("Running Bus event study regression...")
print("=" * 90)
...
# ---------------------------------------------------------------------
# Export full event-study covariance matrix for formal HonestDiD
# ---------------------------------------------------------------------
...
event_terms_for_cov = [event_time_to_term[k] for k in event_times_for_cov]
...
with open(OUT_RESULTS, "w", encoding="utf-8") as f:
    f.write("=" * 90 + "\n")
    f.write("10b Bus Event Study Results\n")
    f.write("=" * 90 + "\n\n")
    ...
    f.write("log_bus_ridership ~ event_time × treated_group\n")
...
plt.title("Bus Event Study: Congestion Pricing Effect")
plt.xlabel("Months Relative to Policy Start (January 2025)")
plt.ylabel("Treatment Effect on Log Bus Ridership")
plt.grid(alpha=0.25)
...
print("=" * 90)
print("Bus Event Study complete")
print("=" * 90)
