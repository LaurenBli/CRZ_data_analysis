...
INPUT = "data/processed/forhire_did_panel.parquet"

OUT_RESULTS = "outputs/models/10d_event_study_forhire_results.txt"
OUT_FIGURE = "outputs/figures/10d_event_study_forhire.png"
OUT_COEFS = "outputs/models/10d_event_study_forhire_coefficients.csv"
OUT_COV = "outputs/models/10d_event_study_forhire_covariance.csv"

os.makedirs("outputs/models", exist_ok=True)
os.makedirs("outputs/figures", exist_ok=True)

print("=" * 90)
print("Loading For-Hire DiD panel...")
print("=" * 90)
...
df["forhire_trips"] = pd.to_numeric(df["forhire_trips"], errors="coerce")
...
df = df[df["forhire_trips"] >= 0].copy()
df["log_forhire_trips"] = np.log1p(df["forhire_trips"])
...
with open(OUT_RESULTS, "w", encoding="utf-8") as f:
    f.write("=" * 90 + "\n")
    f.write("10d For-Hire Event Study Results\n")
    f.write("=" * 90 + "\n\n")
    ...
    f.write("log_forhire_trips ~ event_time × treated_group\n")
    f.write("+ hour FE + day-of-week FE\n")
    f.write("+ holiday_flag + severe_weather_flag\n\n")

    f.write("Treatment definition:\n")
    f.write(
        "treated_group = 1 for FHV pickups in core CRZ taxi zones; "
        "pickup-border zones excluded upstream.\n\n"
    )
    f.write("-" * 90 + "\n")
...
plt.title("For-Hire Event Study: Congestion Pricing Effect")
plt.xlabel("Months Relative to Policy Start (January 2025)")
plt.ylabel("Treatment Effect on Log For-Hire Trips")
plt.grid(alpha=0.25)
